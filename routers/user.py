...
router = APIRouter(prefix="/users/v0", tags=["users"])

# 전체 유저 목록 조회
@router.get("/v0/", response_model=List[UserBase])
...
# 유저 생성 (테스트용)
@router.post("/v0", response_model=UserBase)
...
@router.post("/v0/{student_id}", response_model=UserUpdate)
...
# 유저 삭제 (회원탈퇴 처리)
@router.post("/v0/{student_id}/delete")
def delete_user(student_id: int, db: Session = Depends(get_db)):
    user = db.query(User).filter(User.student_id == student_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    
    user.user_status = UserStatusEnum.DELETED.value
    db.commit()
